        ...
        if (trafficLight != TrafficLight.empty)
        {
            ...
            switch (trafficLight)//Тута сам свет наверн
            {
                ...
                case TrafficLight.red:
                    for (int i = 0; i < TL.Length; i++)
                    {
                        lights1[i].GetComponent<Animator>().SetBool("Isturneron", true);
                        lights2[i].GetComponent<Animator>().SetBool("Isturneron", true);

                        lights1[i].GetComponent<Light>().color = Color.red;
                        lights1[i].transform.localPosition = new Vector3(0, 23, -4);
                        lights2[i].GetComponent<Light>().color = Color.red;
                        lights2[i].transform.localPosition = new Vector3(0, 23, 4);
                    }
                    break;
